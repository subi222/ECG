"""
FCN+DAE Training Script (adapted for WFDB data)

Based on train_deepfilter.py pipeline:
...
def build_dataset_from_records(
    ...
    for rec in records:
        ...
        for target_snr in target_snrs:
            ...
            # FCN+DAE: predict Clean ECG directly (same as DeepFilter)
...
# ============================================================================
# FCN+DAE Model Definition
# ============================================================================

def build_FCN_DAE(signal_size=512):
    """
    FCN+DAE Model (Chiang et al., 2019)
    """
...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default=str(ROOT / 'outputs' / 'train_FCN_DAE'))
    ...
    # Load Model
    print(f"\n[Model] Signal size: {signal_size} samples")
    ...
    model_filepath = str(out_dir / 'FCN_DAE_best.weights.h5')
    ...
    tb_log_dir = str(out_dir / 'logs' / 'FCN_DAE')
