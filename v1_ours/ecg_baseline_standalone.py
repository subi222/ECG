...
def profiler_report(topn: int = 30):
    ...
    print(_border("="));
    [print(_fmt_row(row)) for row in formatted];
    print(_border("-"))
    return rows
        ...
        def wrapped(*args, **kwargs):
            ...
            for a in args:
                if isinstance(a, np.ndarray):
                    key_parts.append(_fingerprint(a))
                else:
                    key_parts.append(a)
            for k, v in sorted(kwargs.items()):
                if isinstance(v, np.ndarray):
                    key_parts.append((k, _fingerprint(v)))
                else:
...
def make_qrs_mask(y, fs=250, rp_ms=150, ts_ms=80, te_ms=350, r_idx=None):
    ...
    if r_idx is None:
        ...
        try:
            r_idx = np.array(nk.ecg_peaks(y, fs)[1].get("ECG_R_Peaks", []), int)
        except:
...
def rr_isoelectric_clamp(y, fs, r_idx=None, t0_ms=80, t1_ms=300):
    ...
    if r_idx is None or len(r_idx) < 2:
        if nk:
            try:
                r_idx = np.array(nk.ecg_peaks(x, fs)[1].get("ECG_R_Peaks", []), int)
            except:
                pass
    if r_idx is None or len(r_idx) < 2: return np.zeros(N)
...
def _baseline_core(y, fs, per_win_s=2.5, per_q=15, asls_lam=5e7, asls_p=0.5, asls_decim=8, qrs_aware=True,
                   verylow_fc=0.55, clamp_win_s=6.0, vol_win_s=0.6, vol_gain=2.0, lam_floor_ratio=0.003, hard_cut=True,
                   break_pad_s=0.30, rr_cap_enable=True, rr_eps_up=5.0, rr_eps_dn=8.0, rr_t0_ms=80, rr_t1_ms=320,
                   r_idx=None, qrs_mask=None, lam_bins=3, min_seg_s=10.0, max_seg_s=60.0):
    x = np.asarray(y, float)
    N = x.size
    if N < 8: return np.zeros_like(x), np.zeros_like(x)
    w0 = int(round(per_win_s * fs));
    w0 += (1 - w0 % 2)
    ...
    def _worker(args):
        ...
        try:
            res = baseline_asls_masked(seg, lam_vec, asls_p, 10, (None if not qrs_aware else base_mask[s:e]),
                                       asls_decim, use32=True)
            return s, e, res
        except:
            res = baseline_asls_masked(seg.astype(float), lam_vec.astype(float), asls_p, 10,
                                       (None if not qrs_aware else base_mask[s:e]), asls_decim, use32=False)
            return s, e, res

    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        results = list(executor.map(_worker, [(s, e, x_res[s:e], lam_smooth[s:e]) for s, e in segs_f if
                                              (e - s) >= max(5, int(0.2 * fs))]))
...
@profiled()
def baseline_hybrid_plus_adaptive(y, fs, per_win_s=2.5, per_q=15, asls_lam=5e7, asls_p=0.5, asls_decim=8,
                                  qrs_aware=True, verylow_fc=0.55, clamp_win_s=6.0, vol_win_s=0.6, vol_gain=2.0,
                                  lam_floor_ratio=0.003, hard_cut=True, break_pad_s=0.30, rr_cap_enable=True,
                                  rr_eps_up=5.0, rr_eps_dn=8.0, rr_t0_ms=80, rr_t1_ms=320, r_idx=None, qrs_mask=None,
                                  lam_bins=3, min_seg_s=10.0, max_seg_s=60.0):
    """
...
@cached_numpy(maxsize=16)
@profiled()
def suppress_negative_sag(y, fs, win_sec=1.0, q_floor=20, k_neg=3.5, min_dur_s=0.25, pad_s=0.25, protect_qrs=True,
                          r_idx=None, qrs_mask=None, use_fast_filter=True):
...
@cached_numpy(maxsize=16)
@profiled()
def fix_downward_steps_mask(y, fs, pre_s=0.5, post_s=0.5, gap_s=0.08, amp_sigma=5.0, amp_abs=None, min_hold_s=0.45,
                            refractory_s=0.80, protect_qrs=True, r_idx=None, qrs_mask=None, smooth_ms=120, hop_ms=10):
    y = np.asarray(y, float)
    N = y.size
    ...
    pr, po, ga, ho, re = int(round(pre_s * fs)), int(round(post_s * fs)), int(round(gap_s * fs)), int(
        round(min_hold_s * fs)), int(round(refractory_s * fs))
...
@cached_numpy(maxsize=16)
@profiled()
def smooth_corners_mask(y, fs, L_ms=140, k_sigma=5.5, protect_qrs=True, r_idx=None, qrs_mask=None, smooth_ms=20,
                        use_float32=True):
...
@cached_numpy(maxsize=16)
@profiled()
def burst_mask(y, fs, win_ms=140, k_diff=7.5, k_std=3.5, pad_ms=80, protect_qrs=True, r_idx=None, qrs_mask=None,
               pre_smooth_ms=0, use_float32=True):
