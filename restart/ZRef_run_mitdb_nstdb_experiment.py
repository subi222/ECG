...
CSV_DIR = OUT_DIR / "csv"
PLOT_DIR = OUT_DIR / "plots_ZRef"

DETAIL_CSV = CSV_DIR / "ZRef_v37_mitdb_nstdb_results_detail.csv"
SUMMARY_CSV = CSV_DIR / "ZRef_v37_mitdb_nstdb_results_summary.csv"
# -------------------------
# IO Helpers
# -------------------------
...
DETAIL_HEADER = [
    ...
    # Z-Ref track
    "z_snr_out_db", "z_snr_improve_db",
    "z_rmse", "z_nrmse_std", "z_prd_percent",
    "zref_offset",
    "n_samples_eval"
]
...
SUMMARY_HEADER = [
    ...
    "prd_mean", "prd_std",
    # ---- 추가: Z-Ref track ----
    "z_snr_out_mean", "z_snr_out_std",
    "z_snr_improve_mean", "z_snr_improve_std",
    "z_rmse_mean", "z_rmse_std",
    "z_nrmse_std_mean", "z_nrmse_std_std",
    "z_prd_mean", "z_prd_std"
]
...
def save_tripanel_plot(rec, ref, noisy_in, corrected, fs, snr_db, zref_method: str = "median"):
    ...
    z_corr = (corrected - offset).astype(np.float32)
...
def main():
    ...
    for rec in records:
        ...
        for snr_tgt in SNR_LEVELS:
            ...
            # ---------- (추가) Z-Ref track 평가 ----------
            # ref 기준으로 0 baseline 정렬 오프셋을 만들고, out에도 동일 오프셋을 적용
            z_ref_eval, z_offset = _compute_zref_zero_baseline(ref_eval, method="median")
            z_out_eval = (out_eval - z_offset).astype(np.float32)

            z_snr_out = float(calculate_snr_db(z_ref_eval, z_out_eval, remove_mean=True))
            z_rmse = float(calculate_rmse(z_ref_eval, z_out_eval))
            z_nrmse = float(calculate_nrmse(z_ref_eval, z_out_eval, mode="std"))
            z_prd = float(calculate_prd(z_ref_eval, z_out_eval, remove_mean=True))

            z_imp = float(z_snr_out - snr_in)

            detail_rows.append([
                rec, NOISE_REC, snr_tgt,
                FS_TARGET,
                # RAW track
                float(snr_in), float(snr_out), float(imp),
                float(rmse), float(nrmse), float(prd),
                # Z-Ref track
                float(z_snr_out), float(z_imp),
                float(z_rmse), float(z_nrmse), float(z_prd),
                float(z_offset),
                N
            ])
    ...
    if detail_rows:
        ...
        # ---- (추가) Z-Ref track 인덱스 ----
        idx_z_snr_out = DETAIL_HEADER.index("z_snr_out_db")
        idx_z_imp = DETAIL_HEADER.index("z_snr_improve_db")
        idx_z_rmse = DETAIL_HEADER.index("z_rmse")
        idx_z_nrmse = DETAIL_HEADER.index("z_nrmse_std")
        idx_z_prd = DETAIL_HEADER.index("z_prd_percent")
        ...
        for noise_rec, snr_tgt in groups:
            ...
            # ---- (추가) Z-Ref track mean/std ----
            z_snr_out_mean, z_snr_out_std = _mean_std([r[idx_z_snr_out] for r in rows_g])
            z_imp_mean, z_imp_std = _mean_std([r[idx_z_imp] for r in rows_g])
            z_rmse_mean, z_rmse_std = _mean_std([r[idx_z_rmse] for r in rows_g])
            z_nrmse_mean, z_nrmse_std = _mean_std([r[idx_z_nrmse] for r in rows_g])
            z_prd_mean, z_prd_std = _mean_std([r[idx_z_prd] for r in rows_g])
            ...
            summary_rows.append([
                ...
                # ---- (추가) Z-Ref track ----
                z_snr_out_mean, z_snr_out_std,
                z_imp_mean, z_imp_std,
                z_rmse_mean, z_rmse_std,
                z_nrmse_mean, z_nrmse_std,
                z_prd_mean, z_prd_std
            ])
